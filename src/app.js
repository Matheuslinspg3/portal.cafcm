  ...
  content.innerHTML = `
    ...
    <section class="card">
      ${apprentices.length ? `<div class="people-list">${apprentices.map((person) => `<article class="person-row apprentice-row">
        ...
        <div class="person-main"><strong>${escapeHtml(person.fullName || "Nome não informado")}</strong><small>${escapeHtml(person.recordOnly ? "E-mail a conferir" : person.email || "E-mail não disponível")}</small><small>${escapeHtml(person.companyId ? companyMap.get(person.companyId) || "Empresa vinculada" : "Sem empresa vinculada")}</small></div>
...
async function renderContracts(content) {
 ...
 content.innerHTML = `${pageHead("Contratos", "Acompanhe vigência e receba alertas nos marcos de 90, 60, 30, 15 e 7 dias.", canManageContracts ? `<button class="btn btn-primary" data-dialog="contract">${icon("plus")} Novo contrato</button>` : "")}
   <section class="metric-grid">${metric("Ativos", activeContracts.length, "calendar")}${metric("Vencem em até 90 dias", due90.length, "history")}${metric("Vencem em até 30 dias", due30.length, "alert")}${metric("Encerrados", allContracts.filter((item) => item.status === "ended").length, "check")}</section>
   <section class="card"><div class="people-list">${allContracts.length ? allContracts.map((item) => {
      const remaining = daysRemaining(item);
     const detail = `${item.imported ? "Importado · " : ""}${companyMap.get(item.company_id) || "Empresa não informada"} · ${formatDate(item.start_date)} a ${formatDate(item.end_date)}${item.position_title ? ` · ${item.position_title}` : ""}${item.course_name ? ` · ${item.course_name}` : ""}${item.work_schedule ? ` · ${item.work_schedule}` : ""}`;
     ...
     const apprenticeName = item.imported ? importedApprenticeMap.get(item.apprentice_id) || "Jovem importado" : apprenticeMap.get(item.apprentice_id) || "Jovem não identificado";
...
async function openDialog(type, recordId = null) {
  ...
  if (type === "registry-apprentice") {
    ...
    if (recordError || !record) throw recordError || new Error("Cadastro importado não encontrado.");
    ...
    body = `<div class="registry-detail">
      <section class="history-person-head"><span class="avatar">${escapeHtml(initials(record.full_name))}</span><div><h3>${escapeHtml(record.full_name)}</h3><p>${escapeHtml(company?.name || "Empresa não informada")}</p></div><span class="status status-draft">Sem acesso</span></section>
      <div class="notice">Este cadastro foi importado sem criar conta. O valor encontrado no campo “Email Aprendiz” foi <strong>${escapeHtml(record.source_email_value || "não informado")}</strong>, por isso o convite não foi enviado.</div>
      <section class="history-metrics"><div><strong>${escapeHtml(registryStatus)}</strong><small>Situação</small></div><div><strong>${escapeHtml(record.cpf || "Não informado")}</strong><small>CPF</small></div><div><strong>${escapeHtml(record.phone || "Não informado")}</strong><small>WhatsApp</small></div><div><strong>${escapeHtml(employment.weekly_hours ? `${employment.weekly_hours} h` : "Não informado")}</strong><small>Jornada semanal</small></div></section>
      <section class="card inset-card"><div class="form-section"><span>Dados do vínculo importado</span></div><div class="form-grid two-columns"><p><small>Início</small><strong>${formatDate(contract?.start_date || "")}</strong></p><p><small>Término</small><strong>${formatDate(contract?.end_date || "")}</strong></p><p><small>Curso</small><strong>${escapeHtml(contract?.course_name || "Não informado")}</strong></p><p><small>Horário</small><strong>${escapeHtml(contract?.work_schedule || employment.work_schedule || "Não informado")}</strong></p><p><small>Status no Airtable</small><strong>${escapeHtml(record.source_status || "Não informado")}</strong></p><p><small>Data de nascimento</small><strong>${formatDate(record.birth_date || "")}</strong></p></div></section>
