Y™Áäx-ÆÈ‹j◊ù¢Îi∫⁄+äßj[hëÈ‹¢ÈÌ◊o{˜ÑËµ©h∫⁄n∂XßzÕZ[\‹ù»‹ôX]P€Y[ùHúõ€Hê›\Xò\ŸK‹›\Xò\ŸKZú»é¬ö[\‹ù»Ÿ]YŸQ›ZYHHúõ€HããŸ›ZY\Àÿÿ][ŸÀõZú»é¬ö[\‹ù»‹ôX]TYŸU›\àHúõ€HããŸ›ZY\À››\ãõZú»é¬ö[\‹ù»ùZ[õ‹õQ›ZYHHúõ€HããŸ›ZY\ÀŸõ‹õ\ÀõZú»é¬Çò€€ú›’TPêT—W’TìHöŒãÀÿﬁ[›õõ[ûúôúYúö]ôãú›\Xò\ŸKò€»é¬ò€€ú›’TPêT—W“—VHHúÿó‹Xõ\⁄XõW€›ô”Ã’íçÃ‹\”öŸû[–Wÿ‘—VMì»é¬ò€€ú›“UW”‘íQ“SàH⁄[ô›Àõÿÿ][€ãõ‹öY⁄[é¬ò€€ú›QRSó—ïSê’S”àH	‘’TPêT—W’TìKŸù[ò›[€úÀ›åK‹‹ù[XYZ[ò¬ò€€ú›UU”PUS”ó—ïSê’S”àH	‘’TPêT—W’TìKŸù[ò›[€úÀ›åK‹‹ù[X]]€X][€ò¬Çò€€ú››\Xò\ŸHH‹ôX]P€Y[ù
...
KàJN¬à€€ú›õŸHH]ÿZ]ô\‹€úŸKöú€€ä
Kòÿ]⁄


HOà
ﬂJJN¬àYà
\ô\‹€úŸKõ⁄ H¬à€€ú›\úõ‹àHô]»\úõ‹äõŸKô\úõ‹àì∞Ë€»õ⁄H‹‹Î]ô[€€ò€Z\àH‹\òpÈË€ÀàäN¬à\úõ‹ãò€ŸHHõŸKò€ŸHù[¬à5”ﬁ˝‚⁄$z{-ÆÈ‹j◊ù= { apprentice_id: values.apprenticeId, company_id: values.companyId, target_start_date: values.targetStartDate || null, status: values.status || "approved", notes: String(values.notes || "").trim() };
      let admissionId = id;
      if (id) {
      ...
      closeOverlay(); showToast(id ? "Admiss√£o atualizada." : "Admiss√£o aberta com checklist."); return renderView();
    }
    ...
    if (form.id === "contract-form") {
      ...
      if (new Date(`${payload.end_date}T00:00:00`) < new Date(`${payload.start_date}T00:00:00`)) throw new Error("A data de t√©rmino n√£o pode ser anterior ao in√≠cio.");
    ...
    if (form.id === "leave-form") {
      ...
      if (new Date(`${payload.end_date}T00:00:00`) < new Date(`${payload.start_date}T00:00:00`)) throw new Error("A data de t√©rmino n√£o pode ser anterior ao in√≠cio.");
      if (payload.actual_return_date && new Date(`${payload.actual_return_date}T00:00:00`) < new Date(`${payload.start_date}T00:00:00`)) throw new Error("A data de retorno n√£o pode ser anterior ao in√≠cio.");
      if (payload.contract_id) {
        ...
        if (contract.apprentice_id !== payload.apprentice_id || (payload.company_id && contract.company_id !== payload.company_id)) throw new Error("O contrato selecionado n√£o pertence ao jovem e √† empresa informados.");
    ...
    if (form.id === "accounting-form") {
      ...
      if (error) throw error; closeOverlay(); showToast("Envio √† contabilidade salvo."); return renderView();
    }
    ...
    if (form.id === "financial-charge-form") {
      ...
      if (paymentSlipUrl && !normalizedPaymentSlipUrl) throw new Error("Informe um link v√°lido para o boleto.");
      const payload = {
      ...
      if (!Number.isFinite(payload.amount) || payload.amount < 0) throw new Error("Informe um valor v√°lido para a cobran√ßa.");
      if (payload.contract_id) {
        const { data: contract, error: contractError } = await supabase.from("contracts").select("company_id").eq("id", payload.contract_id).single();
        if (contractError) throw contractError;
        if (contract.company_id !== payload.company_id) throw new Error("O contrato selecionado n√£o pertence √† empresa informada.");
      }
      ...
      if (error) throw error;
      closeOverlay(); showToast(values.id ? "Cobran√ßa atualizada." : "Cobran√ßa criada."); return renderView();
    }
    ...
    if (form.id === "payable-form") {
      ...
      if (!uniqueReminderDays.length || uniqueReminderDays.length > 5 || uniqueReminderDays.some((value) => value < 0 || value > 60)) {
        throw new Error("Informe de um a cinco lembretes entre 0 e 60 dias, separados por v√≠rgula.");
      }
      const payload = {
      ...
      if (!payload.supplier_name || !payload.category || !payload.description || !payload.due_date || !Number.isFinite(payload.amount) || payload.amount <= 0) {
        throw new Error("Preencha fornecedor, categoria, descri√ß√£o, valor e vencimento.");
      }
      ...
      if (error) throw error;
      closeOverlay(); showToast(values.id ? "Conta a pagar atualizada." : "Conta a pagar criada. Os lembretes ser√£o inclu√≠dos na rotina autom√°tica."); return renderView();
    }
    ...
    if (form.id === "document-requirement-form") {
      if (!values.apprenticeId && !values.companyId) throw new Error("Vincule a pend√™ncia a um jovem ou empresa.");
      ...
      if (error) throw error;
      closeOverlay(); showToast(values.id ? "Pend√™ncia documental atualizada." : "Pend√™ncia documental criada."); return renderView();
    }
    ...
    if (form.id === "email-draft-form") {
      ...
      if (error) throw error;
      closeOverlay(); showToast(values.deliveryId ? "Confer√™ncia salva. O e-mail continua aguardando envio." : "Rascunho salvo. Confira todos os dados antes de enviar."); return renderView();
    }
    ...
    if (form.id === "banking-integration-form") {
      if (values.status === "active" && !window.confirm("O Bradesco confirmou a homologa√ß√£o desta integra√ß√£o?")) throw new Error("Mantenha a situa√ß√£o em homologa√ß√£o at√© a confirma√ß√£o oficial do banco.");
      const payload = {
      ...
      if (error) throw error;
      closeOverlay(); showToast("Prepara√ß√£o banc√°ria salva. Nenhuma opera√ß√£o foi enviada ao banco."); return renderView();
    }
    ...
    if (form.id === "task-form") {
      ...
      if (checklistLines.length > 50) throw new Error("Adicione no m√°ximo 50 itens de checklist por vez.");
      if (checklistLines.length) {
        ...
        if (checklistError) {
          closeOverlay();
          await renderView();
          showToast("A tarefa foi salva, mas o checklist n√£o p√¥de ser inclu√≠do. Abra a tarefa e tente novamente.", "error");
          return;
        }
      }
      closeOverlay();
    ...
    if (form.id === "block-form") {
      ...
      if (!blockTypeLabels[blockType]) throw new Error("Selecione um tipo de linha v√°lido.");
      if (["text", "considerations"].includes(blockType) && !blockContent) throw new Error("Informe o conte√∫do desta linha de aprendizagem.");
      if (blockType === "slides" && !parseSlides(blockContent).length) throw new Error("Adicione ao menos um slide.");
      if (blockType === "video" && !safeHttpUrl(mediaUrl)) throw new Error("Informe uma URL v√°lida para o v√≠deo.");
      if (mediaUrl && !safeHttpUrl(mediaUrl)) throw new Error("A URL de m√≠dia informada n√£o √© v√°lida.");
      const payload = {
      ...
      if (error) throw error;
      closeOverlay();
      showToast("Linha adicionada √† aula.");
      return renderView();
    }
    ...
    if (form.id === "enrollment-form") {
      ...
      if (error?.code === "23505") throw new Error("Este jovem j√° est√° matriculado nesse curso.");
      if (error) throw error;
      closeOverlay();
      showToast("Matr√≠cula criada.");
      return renderView();
    }
...
async function init() {
  ...
  if (tokenHash && otpType) {
    ...
    if (error) {
      ...
      return showToast("O link n√£o p√¥de ser confirmado. Reenvie a confirma√ß√£o e use o novo e-mail recebido.", "error");
    }
